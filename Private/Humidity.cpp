...
float Humidity::GetBaseHumidity(float Latitude, float DistanceToOcean)
{
    ...
    {
        BaseHumidity = FMath::Clamp(90.0f - (DistanceToOcean / 5000.0f), 60.0f, 90.0f);
    }
    else if (AbsoluteLatitude < 40.0f) // Temperate
    {
        BaseHumidity = FMath::Clamp(70.0f - (DistanceToOcean / 10000.0f), 40.0f, 70.0f);
    }
    else if (AbsoluteLatitude < 66.5f) // Sub-Arctic/Grasslands
    {
        BaseHumidity = FMath::Clamp(60.0f - (DistanceToOcean / 15000.0f), 30.0f, 60.0f);
    }
    else // Arctic/Tundra
    {
...
float Humidity::CalculateRelativeHumidity(float Latitude, float DistanceToOcean, bool IsOnshore)
{
    ...
    if (IsOnshore)
    {
        ...
        BaseHumidity += FMath::Clamp(WindEffect - (DistanceToOcean / 10000.0f), 0.0f, 15.0f);
    }
    else
    {
        float WindEffect = FMath::FRandRange(5.0f, 20.0f); // Randomize offshore wind effect
        BaseHumidity -= FMath::Clamp(DistanceToOcean / 500000.0f + WindEffect, 0.0f, 20.0f);
    }
